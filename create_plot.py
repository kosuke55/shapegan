...
class ImageGrid():
    def __init__(
        ...
        self.figure, self.axes = plt.subplots(
            height, width, figsize=(
                width * cell_width, height * cell_height), gridspec_kw={
                'left': 0, 'right': 1, 'top': 1, 'bottom': 0, 'wspace': margin, 'hspace': margin})
    ...
    def set_image(self, image, x=0, y=0):
        cell = self.axes[y,
                         x] if self.height > 1 and self.width > 1 else self.axes[x + y]
        cell.imshow(image)
        cell.axis('off')
...
def create_tsne_plot(
    ...
    if indices is not None:
        ...
        for i in tqdm(range(len(indices))):
            mesh = trimesh.load(
                os.path.join(
                    dataset_directories[index].strip(),
...
if "autodecoder-classes" in sys.argv:
    ...
    dataset_directories = directories = open(
        'data/models.txt', 'r').readlines()
    ...
    for i in range(COUNT):
        ...
        image = render_image(
            sdf_net, latent_codes[i, :], color=dataset.get_color(i), crop=True)
...
if "autoencoder" in sys.argv:
    ...
    create_tsne_plot(codes, voxels, dataset.labels[indices].cpu().numpy(
    ), "plots/{:s}autoencoder-tsne.pdf".format('' if 'classic' in sys.argv else 'variational-'))
...
if "autodecoder_tsne" in sys.argv:
    ...
    create_tsne_plot(
        latent_codes,
        labels=labels,
        filename="plots/deepsdf-tsne.pdf",
        indices=indices)
...
if "autoencoder_hist" in sys.argv:
    ...
    print("Plotting...")
    plt.hist(codes[:, ::4], bins=100, range=(-x_range, x_range), histtype='step',
             density=1, color=['#1f77b4' for _ in range(0, codes.shape[1], 4)])
    plt.xlabel(r"$\mathbf{z}^{(i)}$")
    plt.ylabel("relative abundance")
    plt.savefig("plots/{:s}autoencoder-histogram.pdf".format(
        'variational-' if is_variational else ''), bbox_inches='tight')
    codes = codes.flatten()
    plt.clf()
    ...
    plt.ylabel("relative abundance")
    plt.savefig("plots/{:s}autoencoder-histogram-combined.pdf".format(
        'variational-' if is_variational else ''), bbox_inches='tight')
...
if "autodecoder_hist" in sys.argv:
    ...
    plt.ylabel("relative abundance")
    plt.savefig(
        "plots/autodecoder-histogram-combined.pdf",
        bbox_inches='tight')
...
if "autoencoder_generate" in sys.argv:
    ...
    samples_vae = vae_distribution.sample(
        [SAMPLES, LATENT_CODE_SIZE]).to(device)
    with torch.no_grad():
        reconstructed_ae = ae.decode(samples_ae).cpu().numpy()
        reconstructed_vae = vae.decode(samples_vae).cpu().numpy()

    distances_ae = pairwise_distances(
        codes_ae, samples_ae.cpu().numpy(), metric='cosine')
    indices_ae = np.argmin(distances_ae, axis=0)
    reference_codes_ae = torch.tensor(codes_ae[indices_ae, :], device=device)
    with torch.no_grad():
        reconstructed_references_ae = ae.decode(
            reference_codes_ae).cpu().numpy()

    distances_vae = pairwise_distances(
        codes_vae, samples_vae.cpu().numpy(), metric='cosine')
    indices_vae = np.argmin(distances_vae, axis=0)
    reference_codes_vae = torch.tensor(
        codes_vae[indices_vae, :], device=device)
    with torch.no_grad():
        reconstructed_references_vae = vae.decode(
            reference_codes_vae).cpu().numpy()
...
if "autoencoder_interpolation" in sys.argv:
    ...
    with torch.no_grad():
        ...
        for i in range(STEPS):
            codes_ae[i, :] = code_start * \
                (1.0 - i / (STEPS - 1)) + code_end * i / (STEPS - 1)
...
if "gan_interpolation" in sys.argv:
    ...
    with torch.no_grad():
        ...
        codes_start_end = standard_normal_distribution.sample(
            (2, LATENT_CODE_SIZE))
        code_start = codes_start_end[0, :]
        code_end = codes_start_end[1, :]
        for i in range(STEPS):
...
def get_moving_average(data, window_size):
    ...
    return np.arange(window_size /
                     2, data.shape[0] -
                     window_size /
                     2, dtype=int), moving_average
...
if "sdf_training" in sys.argv:
    ...
    plt.clf()
    plt.plot(np.arange(1, data.shape[0] + 1), data[:, 2],
...
def create_autoencoder_training_plot(data_file, title, plot_file):
    ...
    plt.plot(reconstruction_loss,
             label='Reconstruction loss ({:.3f})'.format(data[-1, 2]))
...
def create_autoencoder_training_plot_latex():
    data = np.genfromtxt(
        'plots/variational_autoencoder_training.csv',
        delimiter=' ')
...
if "autoencoder_training" in sys.argv:
    if 'latex' in sys.argv:
        ...
        create_autoencoder_training_plot(
            'plots/autoencoder_training.csv',
            'Autoencoder Training',
            'plots/autoencoder-training.pdf')
        create_autoencoder_training_plot(
            'plots/variational_autoencoder_training.csv',
            'Variational Autoencoder Training',
            'plots/variational-autoencoder-training.pdf')
...
if "model_images" in sys.argv:
    ...
    for filename in tqdm(filenames):
        model_filename = filename.replace(
            'sdf-pointcloud.npy', 'model_normalized.obj')
...
if 'sdf_net_reconstruction' in sys.argv:
    ...
    for i in range(COUNT):
        ...
        image = render_image_for_index(
            sdf_net, latent_codes, indices[i], crop=True)
        plot.set_image(image, i, 1)
...
if "sdf_net_sample" in sys.argv:
    ...
    mean, variance = np.mean(latent_codes_flattened), np.var(
        latent_codes_flattened) ** 0.5
    print("mean: ", mean)
...
if "hybrid_gan" in sys.argv:
    ...
    codes = standard_normal_distribution.sample(
        [COUNT, LATENT_CODE_SIZE]).to(device)

    plot = ImageGrid(COUNT, create_viewer=False)

    for i in range(COUNT):
        plot.set_image(render_image(generator,
                                    codes[i,
                                          :],
                                    radius=1.6,
                                    crop=True,
...
if "hybrid_gan_interpolation" in sys.argv:
    ...
    codes = standard_normal_distribution.sample(
        [OPTIONS, LATENT_CODE_SIZE]).to(device)
    ...
    for i in range(STEPS):
        plot.set_image(render_image(sdf_net,
                                    codes[i,
                                          :],
                                    crop=True,
                                    radius=1.6,
...
if 'vae_checkpoints' in sys.argv:
    ...
    checkpoints = [i for i in checkpoints if i.startswith(
        'variational-autoencoder-64-')]
    checkpoints = sorted(checkpoints)
    checkpoints = checkpoints[:6]
    checkpoints = [
        checkpoints[i * (len(checkpoints) - 1) // (COUNT - 1)] for i in range(COUNT)]
    print('\n'.join(checkpoints))
...
if 'sdf_checkpoints' in sys.argv:
    ...
    checkpoints = os.listdir(CHECKPOINT_PATH)
    checkpoints_network = [
        i for i in checkpoints if i.startswith('sdf_net-epoch-')]
    checkpoints_latent_codes = [
        i for i in checkpoints if i.startswith('sdf_net_latent_codes-epoch-')]
    checkpoints_network = sorted(checkpoints_network)
    checkpoints_latent_codes = sorted(checkpoints_latent_codes)
    indices = [i * (len(checkpoints_network) - 1) // (COUNT - 1)
               for i in range(COUNT)]
...
if "deepsdf-interpolation-stl" in sys.argv:
    ...
    for i in range(STEPS):
        ...
        mesh = sdf_net.get_mesh(
            codes[i, :], voxel_resolution=256, sphere_only=False)
