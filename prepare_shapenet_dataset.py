...
DIRECTORY_UNIFORM \
    = '/media/kosuke/SANDISK/ShapeNetCore.v2/gan/data/{:s}/uniform/'.format(DATASET_NAME)
DIRECTORY_SURFACE \
    = '/media/kosuke/SANDISK/ShapeNetCore.v2/gan/data/{:s}/surface/'.format(DATASET_NAME)
DIRECTORY_SDF_CLOUD \
    = '/media/kosuke/SANDISK/ShapeNetCore.v2/gan/data/{:s}/cloud/'.format(DATASET_NAME)
DIRECTORY_BAD_MESHES \
    = '/media/kosuke/SANDISK/ShapeNetCore.v2/gan/data/{:s}/bad_meshes/'.format(DATASET_NAME)

# Voxel resolutions to create.
# Set to [] if no voxels are needed.
# Set to [32] for for all models except for the progressively growing
# DeepSDF/Voxel GAN
...
# For uniform and surface points (CREATE_UNIFORM_AND_SURFACE)
...
def get_voxel_filename(model_filename, resolution):
    return os.path.join(
        DIRECTORY_VOXELS.format(resolution),
        get_hash(model_filename) + '.npy')
...
def get_uniform_and_surface_points(
        surface_point_cloud,
    ...
    near_surface_points = surface_points + \
        np.random.normal(
            scale=0.0025, size=surface_points.shape).astype(np.float32)
    near_surface_sdf = surface_point_cloud.get_sdf(
        near_surface_points, use_depth_buffer=USE_DEPTH_BUFFER)
...
def process_model_file(filename):
    try:
        ...
        voxel_filenames = [get_voxel_filename(
            filename, resolution) for resolution in VOXEL_RESOLUTIONS]
            ...
            try:
                for resolution in VOXEL_RESOLUTIONS:
                    voxels = surface_point_cloud.get_voxels(
                        resolution, use_depth_buffer=USE_DEPTH_BUFFER, check_result=True)
                    np.save(get_voxel_filename(filename, resolution), voxels)
                    del voxels

            except BadMeshException:
                tqdm.write("Skipping bad mesh. ({:s})".format(
                    get_hash(filename)))
        ...
        if create_uniform_and_surface or create_sdf_clouds:
            ...
            try:
                if create_uniform_and_surface:
                    ...
                    combined_uniform = np.concatenate(
                        (uniform_points, uniform_sdf[:, np.newaxis]), axis=1)
                    np.save(get_uniform_filename(filename), combined_uniform)

                    combined_surface = np.concatenate(
                        (near_surface_points, near_surface_sdf[:, np.newaxis]), axis=1)
                    np.save(get_surface_filename(filename), combined_surface)

                if create_sdf_clouds:
                    sdf_points, sdf_values = surface_point_cloud.sample_sdf_near_surface(
                        use_scans=True, sign_method='depth' if USE_DEPTH_BUFFER else 'normal', number_of_points=SDF_POINT_CLOUD_SIZE, min_size=0.015)
                    combined = np.concatenate(
                        (sdf_points, sdf_values[:, np.newaxis]), axis=1)
...
def combine_sdf_clouds():
    ...
    for file_name in tqdm(files):
        ...
        points[position *
               SDF_POINT_CLOUD_SIZE:(position +
                                     1) *
               SDF_POINT_CLOUD_SIZE, :] = torch.tensor(numpy_array[:, :3])
        sdf[position * SDF_POINT_CLOUD_SIZE:(position + 1) *
            SDF_POINT_CLOUD_SIZE] = torch.tensor(numpy_array[:, 3])
        del numpy_array
        position += 1
