...
def get_npy_filename(model_filename, qualifier=''):
    return DIRECTORY_SDF + \
        model_filename[len(DIRECTORY_MODELS):-len(MODEL_EXTENSION)] + qualifier + '.npy'


def get_voxel_filename(model_filename):
    return get_npy_filename(model_filename,
                            '-voxels-{:d}'.format(VOXEL_RESOLUTION))
...
def get_bad_mesh_filename(model_filename):
    return DIRECTORY_SDF + \
        model_filename[len(DIRECTORY_MODELS):-len(MODEL_EXTENSION)] + '.badmesh'
    ...
    if CREATE_SDF_CLOUDS:
        try:
            points, sdf = surface_point_cloud.sample_sdf_near_surface(
                number_of_points=SDF_CLOUD_SAMPLE_SIZE, sign_method='depth', min_size=0.015)
...
def combine_pointcloud_files():
    ...
    for npy_filename in tqdm(npy_files):
        ...
        points[position *
               SDF_CLOUD_SAMPLE_SIZE:(position +
                                      1) *
               SDF_CLOUD_SAMPLE_SIZE, :] = torch.tensor(numpy_array[:, :3])
        sdf[position *
            SDF_CLOUD_SAMPLE_SIZE:(position +
                                   1) *
            SDF_CLOUD_SAMPLE_SIZE] = torch.tensor(numpy_array[:, 3])
