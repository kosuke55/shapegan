...
# Otherwise, it will use the model parameters of the previous iteration or
# initialize randomly at iteration 0
...
if torch.cuda.device_count() > 1:
    print(
        "Using dataparallel with {:d} GPUs.".format(
...
def get_gradient_penalty(real_sample, fake_sample):
    alpha = torch.rand(
        (real_sample.shape[0], 1, 1, 1), device=device).expand(
        real_sample.shape)
    ...
    gradients = autograd.grad(
        ...
        grad_outputs=torch.ones(
            discriminator_output.shape).to(device),
        create_graph=True,
        retain_graph=True,
        only_inputs=True)[0]
    return ((gradients.norm(2, dim=(1, 2, 3)) - 1)
            ** 2).mean() * GRADIENT_PENALTY_WEIGHT
...
def train():
    ...
    for epoch in range(first_epoch, NUMBER_OF_EPOCHS):
        progress.desc = 'Epoch {:d}/{:d} ({:d}³)'.format(
            epoch, NUMBER_OF_EPOCHS, VOXEL_RESOLUTION)
        batch_index = 0
        ...
        for valid_sample in data_loader:
            try:
                ...
                if current_batch_size == BATCH_SIZE:
                    ...
                    batch_grid_points = grid_points.repeat(
                        (current_batch_size, 1))

                if not CONTINUE and ITERATION > 0:
                    discriminator.fade_in_progress = (
                        epoch + batch_index / (len(dataset) / BATCH_SIZE)) / FADE_IN_EPOCHS
                ...
                if batch_index % 5 == 0:
                    ...
                    fake_sample = generator_parallel(
                        batch_grid_points, latent_codes)
                ...
                gradient_penalty = get_gradient_penalty(
                    valid_sample.detach(), fake_sample.detach())
                loss = discriminator_output_fake.mean() - discriminator_output_valid.mean() + \
                    gradient_penalty
                loss.backward()
        ...
        tqdm.write(
            'Epoch {:d} ({:.1f}s), D(x\'): {:.4f}, D(x): {:.4f}, loss: {:4f}, gradient penalty: {:.4f}'.format(
                epoch,
                time.time() -
                epoch_start_time,
                prediction_fake,
        ...
        if "show_slice" in sys.argv:
            ...
            slice_voxels = slice_voxels.reshape(
