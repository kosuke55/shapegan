...
def create_text_slice(voxels):
    ...
    lines = ['|' + ''.join([CHARACTERS[i]
                            for i in line]) + '|' for line in data]
    result = []
...
def crop_image(image, background=255):
    ...
    half_size = int(
        max(bottom_right[0] - top_left[0], bottom_right[1] - top_left[1]) / 2)
    center = ((top_left + bottom_right) / 2).astype(int)
    center = (min(max(half_size,
                      center[0]),
                  image.shape[0] - half_size),
              min(max(half_size,
                      center[1]),
                  image.shape[1] - half_size))
    if half_size > 100:
        image = image[center[0] - half_size: center[0] +
                      half_size, center[1] - half_size: center[1] + half_size]
    return image
