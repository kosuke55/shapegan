    ...
    for i in tqdm(range(sample_count)):
        try:
            point_cloud = sdf_net.get_uniform_surface_points(
                latent_codes[i, :], point_count=point_cloud_size, voxel_resolution=voxel_resolution, sphere_only=False, level=LEVEL)
...
def sample_from_voxels(voxels, point_cloud_size, rescale='half_unit_sphere'):
    ...
    for i in tqdm(range(voxels.shape[0])):
        ...
        vertices, faces, normals, _ = skimage.measure.marching_cubes_lewiner(
            voxels_current, level=0, spacing=(
...
if 'dataset' in sys.argv:
    ...
    dataset = VoxelDataset.from_split(
        'data/airplanes/voxels_64/{:s}.npy',
