...
class SDFVoxelizationHelperData():
    def __init__(self, device, voxel_resolution, sphere_only=True):
        ...
        if sphere_only:
            ...
            self.unit_sphere_mask = unit_sphere_mask.reshape(
...
class SDFNet(SavableModule):
    def __init__(self, latent_code_size=LATENT_CODE_SIZE, device='cuda'):
        ...
        self.layers1 = nn.Sequential(
            nn.Linear(in_features=3 + latent_code_size,
                      out_features=SDF_NET_BREADTH),
            nn.ReLU(inplace=True),

            nn.Linear(in_features=SDF_NET_BREADTH,
                      out_features=SDF_NET_BREADTH),
            nn.ReLU(inplace=True),
        ...
        self.layers2 = nn.Sequential(
            nn.Linear(in_features=SDF_NET_BREADTH +
                      latent_code_size + 3, out_features=SDF_NET_BREADTH),
            nn.ReLU(inplace=True),
    ...
    def evaluate_in_batches(self, points, latent_code, batch_size=100000, return_cpu_tensor=True):
        ...
        with torch.no_grad():
            ...
            for i in range(batch_count):
                result[batch_size * i:batch_size * (i+1)] = self(
                    points[batch_size * i:batch_size * (i+1), :], latent_codes)
            remainder = points.shape[0] - batch_size * batch_count
            result[batch_size * batch_count:] = self(
                points[batch_size * batch_count:, :], latent_codes[:remainder, :])
        return result
    ...
    def get_voxels(self, latent_code, voxel_resolution, sphere_only=True, pad=True):
        if not (voxel_resolution, sphere_only) in sdf_voxelization_helper:
            helper_data = SDFVoxelizationHelperData(
                self.device, voxel_resolution, sphere_only)
            sdf_voxelization_helper[(
                voxel_resolution, sphere_only)] = helper_data
        else:
            helper_data = sdf_voxelization_helper[(
                voxel_resolution, sphere_only)]

        with torch.no_grad():
            distances = self.evaluate_in_batches(
                helper_data.sample_points, latent_code).numpy()

        if sphere_only:
            voxels = np.ones((voxel_resolution, voxel_resolution,
                              voxel_resolution), dtype=np.float32)
            voxels[helper_data.unit_sphere_mask] = distances
        else:
            voxels = distances.reshape(
                voxel_resolution, voxel_resolution, voxel_resolution)
            if pad:
    ...
    def get_mesh(self, latent_code, voxel_resolution=64, sphere_only=True, raise_on_empty=False, level=0):
        ...
        voxels = self.get_voxels(
            latent_code, voxel_resolution=voxel_resolution, sphere_only=sphere_only)
        voxels = np.pad(voxels, 1, mode='constant', constant_values=1)
        try:
    ...
    def get_uniform_surface_points(self, latent_code, point_count=1000, voxel_resolution=64, sphere_only=True, level=0):
        mesh = self.get_mesh(
            latent_code, voxel_resolution=voxel_resolution, sphere_only=sphere_only, level=level)
    ...
    def get_normals(self, latent_code, points):
        if latent_code.requires_grad or points.requires_grad:
            raise Exception(
                'get_normals may only be called with tensors that don\'t require grad.')
    ...
    def get_surface_points(self, latent_code, sample_size=100000, sdf_cutoff=0.1, return_normals=False, use_unit_sphere=True):
        if use_unit_sphere:
            points = get_points_in_unit_sphere(
                n=sample_size, device=self.device) * 1.1
        else:
        ...
        mask = (torch.abs(sdf) < sdf_cutoff) & torch.all(
            torch.isfinite(points), dim=1)
        points = points[mask, :]
    ...
    def get_surface_points_in_batches(self, latent_code, amount=1000):
        ...
        while position < amount and iteration_limit > 0:
            points = self.get_surface_points(
                latent_code, sample_size=amount * 6)
